...
import { articleAdd, articleDeleteImage, articleUpdate } from '../../../redux/slice/articleSlice';
...
const removeImage = (file, index) => {
  // If image already exists in DB (edit mode)
  if (!file.preview && dialogueData) {
    dispatch(
      articleDeleteImage({
        articleId: dialogueData._id,
        position: index, // IMPORTANT: backend uses position
      })
    );
  }
